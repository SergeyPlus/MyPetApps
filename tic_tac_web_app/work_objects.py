...
class Game:
    ...
    def _complete_field(self, turn_dict: Dict) -> None:
        ...
        for key, value in turn_dict.items():
            if value:
                ...
                self.game_data['tic_tac_table'][key] = f'__{value}__'
                wo_logger.info(f"The turn was recorded in Game successfully {self.game_data.get('tic_tac_table')}")
                break
        ...
        if player_turn.lower() == Constants.SYM_CROSS.value:
            self.game_data['game_field'][node_index] = Constants.CROSS.value
            wo_logger.info(f'Value to the game_field list - {1}')
        elif player_turn == Constants.SYM_ZERO.value:
            self.game_data['game_field'][node_index] = Constants.ZERO.value
    ...
    def _horizontal_check(self) -> List[int]:
        ...
        summ: int = 0
        for i in range(0, len(self.game_data['game_field']), 3):
            for k in range(3):
                k += i
                summ += self.game_data['game_field'][k]
                if k == (i + 2):
    ...
    def _vertical_check(self) -> List[int]:
        ...
        summ: int = 0
        for i in range(3):
            for k in range(0, len(self.game_data['game_field']), 3):
                k += i
                summ += self.game_data['game_field'][k]
                if k == (i + 6):
    ...
    def _diagonals_check(self) -> Tuple:
        ...
        diagonal_check_1 = sum([self.game_data['game_field'][i] for i in (0, 4, 8)])
        wo_logger.info(f'Calculated diagonal_1 {diagonal_check_1}')

        diagonal_check_2 = sum([self.game_data['game_field'][i] for i in (2, 4, 6)])
    ...
    def _check_winner(self) -> None:
        ...
        wo_logger.info(f'Before win-win checking. {self.game_data}')
        if not self.game_data['winner'] and len(self.game_data.get('tic_tac_table', {})) == 9:
    ...
    def _identify_winner(self, sym) -> None:
        ...
        if sym == players.players_data['sym_1']:
            self.game_data['winner'] = players.players_data['name_1']
            players.players_data['score_1'] += 1
        elif sym == players.players_data['sym_2']:
            self.game_data['winner'] = players.players_data['name_2']
            players.players_data['score_2'] += 1
        else:
            players.players_data['score_1'] += 1
            players.players_data['score_2'] += 1
            self.game_data['winner'] = f" both players {players.players_data['name_1']} and {players.players_data['name_2']} win"
        wo_logger.info(
    ...
    def _finalize_field(self) -> None:
        ...
        """
        for key, value in self.game_data['tic_tac_table'].items():
            wo_logger.info(f'From __finalize_field {key} , {value}')
            if not value:
                self.game_data['tic_tac_table'][key] = '_____'
        wo_logger.info(f'Finalizing field starting. From now all field cells are completed {self.game_data}')
...
class GameManager:
    ...
    def __getitem__(self, key: str):
        wo_logger.info(f"Getting item - {key}")
        if key == 'game_data':
            return self.game.game_data
        elif key == 'players_data':
            return self.players.players_data
